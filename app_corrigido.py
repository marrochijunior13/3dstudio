...
def depth_to_stl(depth: np.ndarray, scale_z: float = 0.4) -> bytes:
    ...
    for y in range(h - 1):
        for x in range(w - 1):
            def v(yy, xx):
                return ((xx/(w-1))*2-1, -(((yy/(h-1))*2-1)), float(depth[yy,xx])*scale_z)
            a, b = v(y,x), v(y,x+1)
    ...
    top_edge   = [(x/(w-1)*2-1, 1.0, float(depth[0,x])*scale_z) for x in range(w)]
    bot_edge   = [(x/(w-1)*2-1,-1.0, float(depth[h-1,x])*scale_z) for x in range(w)]
    left_edge  = [(-1.0, -(y/(h-1)*2-1), float(depth[y,0])*scale_z) for y in range(h)]
    right_edge = [( 1.0, -(y/(h-1)*2-1), float(depth[y,w-1])*scale_z) for y in range(h)]
    def add_side(edge, nx, ny):
        for i in range(len(edge)-1):
            t, bl = edge[i], (edge[i][0],edge[i][1],base_z)
            t2, br = edge[i+1], (edge[i+1][0],edge[i+1][1],base_z)
            norm = (nx, ny, 0.0)
            triangles.append((norm,t,bl,t2))
            triangles.append((norm,t2,bl,br))
    add_side(top_edge,0,1); add_side(bot_edge,0,-1)
    add_side(left_edge,-1,0); add_side(right_edge,1,0)
    header = b"3D Studio STL" + b'\x00'*(80-13)
...
# ═══════════════════════════════════════════════════════════════════
#  WIDGET AUXILIAR — botão estilizado (sem Canvas para evitar conflitos)
# ═══════════════════════════════════════════════════════════════════
class RoundedButton(tk.Button):
    ...
    def set_state(self, state):
        if state == "disabled":
            self._disabled = True
            self.configure(bg=MUTED, state="disabled", cursor="arrow")
        else:
            self._disabled = False
            self._normal_bg = ACCENT
            self._hover_bg  = self._darken(ACCENT)
            self.configure(bg=ACCENT, fg=WHITE, state="normal", cursor="hand2")
...
class Studio3D(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("3D Studio — Conversor 2D para 3D")
        ...
        self.status_var = tk.StringVar(value="Pronto — selecione uma imagem para comecar")
    ...
    def _build_navbar(self):
        ...
        tk.Label(logo, text="3D", font=("Segoe UI",16,"bold"), fg=YELLOW, bg="#111116").pack(side="left")
        tk.Label(logo, text=" STUDIO", font=("Segoe UI",13,"bold"), fg=WHITE, bg="#111116").pack(side="left")
        tk.Label(logo, text="  v1.2", font=FONT_SMALL, fg=MUTED, bg="#111116").pack(side="left")
        for lbl in ["Converter","Galeria","Sobre"]:
            tk.Label(nav, text=lbl, font=FONT_BODY, fg=MUTED, bg="#111116", cursor="hand2").pack(side="left", padx=12)
        tk.Label(nav, text="Powered by IA", font=FONT_SMALL, fg=YELLOW, bg="#111116").pack(side="right", padx=18)
    ...
    def _build_left(self, parent):
        ...
        self.drop_icon = tk.Label(self.drop_zone, text="[IMG]", font=("Segoe UI",24),
                                  fg=MUTED, bg="#13131a")
        ...
        for fmt in [("GLB", ACCENT), ("OBJ", ACCENT), ("STL", GREEN)]:
            tk.Radiobutton(r1, text=fmt[0], variable=self.fmt_var, value=fmt[0],
        ...
        tk.Label(r3, text="Profund. Z:", font=FONT_LABEL, fg=MUTED, bg=CARD, width=11, anchor="w").pack(side="left")
        ...
        self.conv_btn = RoundedButton(btn_f, text="Converter para 3D",
                                      ...
                                      bg=MUTED, font=("Segoe UI",11,"bold"))
        self.conv_btn.pack(fill="x", ipady=6)
        self.conv_btn.set_state("disabled")

        # Progress bar
        prog_f = tk.Frame(parent, bg=CARD)
        prog_f.pack(fill="x", padx=18, pady=(2,4))
        self.prog_canvas = tk.Canvas(prog_f, height=6, bg=BORDER, highlightthickness=0)
        self.prog_canvas.pack(fill="x", pady=(2,3))
        self.prog_fill_id = self.prog_canvas.create_rectangle(0,0,0,6, fill=ACCENT, outline="")
    ...
    def _build_right(self, parent):
        ...
        self.viewer_ph = tk.Label(self.viewer,
            text="O mapa de profundidade\naparecera aqui apos a conversao",
        ...
        stats_f = tk.Frame(parent, bg="#13131a")
        stats_f.pack(fill="x", padx=18, pady=(0,8))
        self.stats_lbl = tk.Label(stats_f, text="Aguardando conversao...",
        ...
        self.dl_btn = RoundedButton(dl_f, text="Salvar Modelo",
                                    command=self._save_model, bg=MUTED)
        self.dl_btn.pack(side="left", padx=(0,8), ipady=4)
        self.dl_btn.set_state("disabled")
        self.folder_btn = RoundedButton(dl_f, text="Abrir Pasta",
                                        command=self._open_folder, bg="#333344")
        self.folder_btn.pack(side="left", ipady=4)

        feat = tk.Frame(parent, bg="#141420")
        ...
        items = [
            ("GLB",  "GLTF binario - Unity, Blender, Three.js, Sketchfab"),
            ("OBJ",  "Universal - com textura e material (.mtl)"),
            ("STL",  "Impressao 3D - Cura, PrusaSlicer, Tinkercad"),
        ]
    ...
    def _redraw_prog(self, event=None):
        try:
            ...
            fill_w = int(w * self.prog_var.get())
    ...
    def _pick_image(self):
        ...
        try:
            ...
            self.img_label.configure(image=tk_thumb, text="")
            self.img_label._img = tk_thumb
            self.img_label.pack(expand=True, pady=10)
            self.conv_btn.set_state("normal")
            self.status_var.set(f"Imagem: {os.path.basename(path)}  |  {pil.width}x{pil.height} px")
    ...
    def _convert_thread(self):
        try:
            ...
            stats = (f"Modo: {self.mode_var.get()}  |  Formato: {fmt}  |  "
                     f"Vertices: {verts:,}  |  Faces: {faces:,}  |  "
                     f"Tamanho: {sz//1024} KB  |  Z: {z:.2f}")

            def done():
                self._set_progress(1.0, "Concluido!")
                self.viewer_ph.pack_forget()
                self.viewer_img_lbl.configure(image=tk_depth)
                self.viewer_img_lbl._img = tk_depth
            ...
            def err():
                ...
                self.conv_btn.set_state("normal")
                messagebox.showerror("Erro na conversao", str(e))
            self.after(0, err)
